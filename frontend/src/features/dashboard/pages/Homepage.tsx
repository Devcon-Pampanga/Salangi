...
function Homepage() {
  ...
  // Cache user coords when nearMe is toggled on
  useEffect(() => {
  ...
  const handleListBusinessClick = async () => {
    ...
    try {
      ...
      if (!user) { navigate(ROUTES.LIST_YOUR_BUSINESS); return; }
      const { data: userListings } = await supabase
        .from('listings').select('id').eq('user_id', user.id).limit(1);
  ...
  useEffect(() => {
    const fetchSaves = async () => {
      try {
        ...
        if (!user) return;
        const { data, error } = await supabase.from('saves').select('listing_id').eq('user_id', user.id);
        if (!error && data) setSavedIds(data.map((row: any) => row.listing_id));
      } catch (error) {
  ...
  const filteredListings = useMemo<Listing[]>(() => {
    let result = listings.filter((item: Listing) => {
      ...
      return matchesCategory && matchesSearch && matchesRating && matchesOpenNow && matchesNearMe;
    });
    ...
    return result;
  }, [listings, activeCategory, searchQuery, filters, averageRatings, userCoords]);
  ...
  return (
    <div className="relative w-full h-full bg-[#1A1A1A] text-[#FBFAF8] overflow-hidden">
      ...
      {isRedirecting && createPortal(
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-6" style={{ backgroundColor: '#1A1A1A' }}>
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl opacity-30 pointer-events-none"
            style={{ width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(255,226,160,0.8) 0%, rgba(255,226,160,0.1) 60%, transparent 80%)' }}
          />
      ...
      <div className="absolute top-0 left-0 rounded-full blur-3xl opacity-60 pointer-events-none hidden md:block"
        style={{ width: '760px', height: '680px', transform: 'translate(-400px, -440px)', background: 'radial-gradient(circle, rgba(255,226,160,0.8) 0%, rgba(255,226,160,0.2) 50%, transparent 70%)' }}
      />
      ...
      <div className="relative z-10 h-full flex flex-col md:flex-row px-4 py-4 md:px-6 md:py-6 gap-4 md:gap-6 overflow-y-auto md:overflow-hidden">
        ...
        <div className="md:hidden flex items-center justify-between w-full shrink-0 relative z-50 order-first">
          <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 -ml-2 text-[#FFE2A0] hover:bg-[#FFE2A0]/10 rounded-lg transition-colors cursor-pointer">
            <Menu size={28} />
          </button>
        </div>
        ...
        {isMobileMenuOpen && createPortal(
          <div className="fixed inset-0 z-9999 bg-[#1A1A1A] p-6 flex flex-col gap-8 md:hidden">
            <div className="flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 -mr-2 text-[#FBFAF8]/70 hover:text-[#FFE2A0] rounded-lg transition-colors cursor-pointer">
                <X size={28} />
              </button>
            </div>
            <div className="flex flex-col gap-6 flex-1">
              <div className="flex flex-col gap-2">
                ...
                <SearchBar placeholder="Explore local spots" value={searchQuery} onChange={(e) => handleSearchChange(e)} className="py-3 w-full" containerClassName="w-full shadow-lg" onFilterChange={setFilters} filters={filters} />
              </div>
              <div className="h-px w-full bg-[#373737]/50" />
              <button onClick={() => { setIsMobileMenuOpen(false); handleListBusinessClick(); }} className="flex items-center justify-center gap-2 px-4 py-3.5 bg-[#FFE2A0] text-[#1A1A1A] rounded-xl font-bold text-md w-full shadow-lg active:scale-95 transition-all cursor-pointer">List Your Business</button>
              <button onClick={() => { setIsMobileMenuOpen(false); setIsSettingsOpen(true); }} className="flex items-center justify-center gap-3 px-4 py-3.5 bg-[#373737] text-[#FBFAF8] rounded-xl font-semibold text-md w-full shadow-lg active:scale-95 transition-all cursor-pointer"><Settings size={20} className="text-[#FFE2A0]" />Settings</button>
              <button onClick={() => { setIsMobileMenuOpen(false); handleLogout(); }} className="flex items-center justify-center gap-3 px-4 py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl font-semibold text-md w-full shadow-lg active:scale-95 transition-all cursor-pointer mt-auto"><LogOut size={20} className="opacity-90" />Log out</button>
            </div>
          </div>,
          document.body
        )}
        ...
        <div className="flex flex-col shrink-0 w-full md:w-120 md:h-full overflow-visible md:overflow-hidden order-2 md:order-1">
          <div className="shrink-0 mb-4 md:mb-0">
            <h1 className="font-['Playfair_Display'] text-6xl md:text-3xl leading-tight mt-4 md:mt-0 text-[#FFE2A0]">Salangi</h1>
            <p className="mb-5 text-lg font-['Playfair_Display'] tracking-wide">Bring <span className="text-[#FFE2A0]">light</span> to my <span className="text-[#FFE2A0]">home!</span></p>
            <CategoryFilters activeCategory={activeCategory} onCategoryChange={handleCategoryChange} className="mb-5" />
          </div>
        ...
        <div className="hidden md:flex flex-col flex-none md:flex-1 w-full overflow-visible min-w-0 min-h-0 relative z-50 order-1 md:order-2 space-y-4 md:space-y-0">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-end gap-3 shrink-0">
            <SearchBar placeholder="Explore local spots" value={searchQuery} onChange={handleSearchChange} className="py-1 w-full" containerClassName="w-full md:w-80" onFilterChange={setFilters} filters={filters} />
            <button onClick={handleListBusinessClick} className="flex items-center justify-center gap-2 px-4 py-3 bg-[#FFE2A0] text-[#1A1A1A] rounded-lg font-semibold text-sm whitespace-nowrap cursor-pointer hover:bg-[#f5d880] transition-colors w-full md:w-auto">List Your Business</button>
          </div>
          <div className="hidden md:block w-full h-75 md:h-auto md:flex-1 md:mt-2 md:min-h-0">
            <div className="w-full h-full rounded-2xl overflow-hidden">
              <MapView listings={filteredListings} selectedListing={selectedListing} onSelect={handleCardSelect} />
            </div>
          </div>
        </div>
      ...
      {isSettingsOpen && createPortal(<SettingsPage onClose={() => setIsSettingsOpen(false)} />, document.body)}
    </div>
  );
}
