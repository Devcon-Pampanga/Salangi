...
// ── Geocode helper (same as ListBusiness) ─────────────────────────────────────
...
// ── Draggable Map (same pattern as ListBusiness) ──────────────────────────────
...
function DraggableMap({ lat, lng, onPinMove }: DraggableMapProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        ref={mapRef}
        style={{ height: "200px", width: "100%", borderRadius: "10px", overflow: "hidden" }}
      />
      <p className="text-xs text-center" style={{ color: "#666666" }}>
        📍 Drag the pin to set the exact location of your event
      </p>
    </div>
  );
}
...
export default function PostEventModal({
  ...
  // ── Geocode when city/barangay changes (mirrors ListBusiness) ──────────────
  useEffect(() => {
    if (!form.city) {
      setLat(null);
      setLng(null);
    ...
    geocodeTimeout.current = setTimeout(async () => {
      ...
      if (!coords)
        coords = CITY_COORDS[form.city] ?? { lat: 15.145, lng: 120.5887 };
      setLat(coords.lat);
      setLng(coords.lng);
      setGeocoding(false);
    }, 800);
  ...
  const handleSubmit = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const effectiveDateFrom =
        form.dateFrom || (editEvent ? "2026-05-02" : "");
      const dateObj = new Date(effectiveDateFrom);
      const month = dateObj.toLocaleString("en-US", { month: "short" });
      const day = dateObj.getDate().toString();
      const timeDisplay = `${form.timeFrom || ""}${
      ...
      const dateRange =
        form.dateTo && form.dateTo !== form.dateFrom
          ...
          : effectiveDateFrom;
      const locationDisplay = [form.barangay, form.city]
        .filter(Boolean)
        .join(", ");
      onAddEvent({
        id: editEvent?.id,
  ...
  return (
    <>
      {/* Inject option color fix globally (options ignore CSS vars in many browsers) */}
      <style>{`
        .event-modal-select option {
          background-color: #2a2a2a;
          color: #e8e8e8;
        }
        .event-modal-select option:checked,
        .event-modal-select option:hover {
      ...
      >
        ...
        >
          ...
          <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
            ...
            <div>
              <label
                className="block font-normal mb-1.5 text-md tracking-wide"
                style={{ color: "#FFE2A0" }}
              >
                Event Image
              </label>
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    ...inputBase,
                    colorScheme: "dark",
                    color: form.dateFrom ? "#e8e8e8" : "#666666",
                  }}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  style={{
                    ...inputBase,
                    colorScheme: "dark",
                    color: form.dateTo ? "#e8e8e8" : "#666666",
                  }}
                />
              </div>
            </div>
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    ...inputBase,
                    colorScheme: "dark",
                    color: form.timeFrom ? "#e8e8e8" : "#666666",
                  }}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  style={{
                    ...inputBase,
                    colorScheme: "dark",
                    color: form.timeTo ? "#e8e8e8" : "#666666",
                  }}
                />
              </div>
            </div>

            {/* ── Location: City + Barangay dropdowns → then map ── */}
            <div>
              <label
                className="block font-normal mb-1.5 text-md tracking-wide"
                style={{ color: "#FFE2A0" }}
              ...
              {/* Barangay — only shown once a city is selected */}
              {form.city && (
                <select
                  value={form.barangay}
                  onChange={(e) =>
                  ...
                  style={{
                    ...inputBase,
                    color: form.barangay ? "#e8e8e8" : "#666666",
                  }}
                >
                  <option value="" style={{ color: "#666666" }}>
                    Select barangay (optional)
                  </option>
                  {LOCATIONS[form.city]?.map((brgy: string) => (
              ...
              {/* Draggable map — mirrors ListBusiness behaviour */}
              {lat !== null && lng !== null && (
                <div className="mt-1">
                  {geocoding && (
                    <p
                      className="text-xs mb-2 animate-pulse"
                      style={{ color: "rgba(255,226,160,0.7)" }}
                    >
                      📡 Finding location…
                    </p>
                  )}
              ...
              onMouseEnter={(e) => {
                if (!submitting) {
                  ...
                  e.currentTarget.style.boxShadow =
                    "0 4px 16px rgba(255,226,160,0.25)";
                }
              }}
              onMouseLeave={(e) => {
